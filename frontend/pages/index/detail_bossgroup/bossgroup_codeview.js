...
Page({
            ...
            success: function(res) {
                ...
                wx.request({
                    ...
                    success: function(res) {
                        ...
                        info.book_info.forEach(function(elem, idx) {
                            var item = info.book_info[idx];
                            if (item.is_used == 1) return;
                            QR.qrApi.draw(item.order_code, 'qrcanvas' + idx, _sW * .5, _sW * .5, that, idx);
                        });
                    },
                    complete: function() {
                        wx.hideLoading({});
    ...
    previewImage: function(e) {
        var imageFile = e.currentTarget.dataset.src;
        wx.previewImage({
            current: imageFile, // 当前显示图片的http链接  
            urls: [imageFile] // 需要预览的图片http链接列表  
        });
